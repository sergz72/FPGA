...
public class ParserTests
{
    ...
    public void ParserTest()
    {
        var parser = new ForthParser([new ParserFile("file.f", [
            "1234 abcde hex abcde s\" text, text2\" ."
        ])]);
        ...
        Assert.That(tokens[3].Type, Is.EqualTo(TokenType.Word));
        Assert.That(tokens[3].Word, Is.EqualTo("s\""));
        Assert.That(tokens[3].StringValue, Is.Not.Null);
        Assert.That(tokens[3].StringValue, Is.EqualTo("text, text2"));
