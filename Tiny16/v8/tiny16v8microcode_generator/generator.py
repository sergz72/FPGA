...
REGISTERS_WR_SOURCE_SET = allocate_bit(2)
REGISTERS_WR_SOURCE_SET127 = 2 * REGISTERS_WR_SOURCE_SET
...
REGISTER_BANK_OP = allocate_bit(2)
REGISTER_BANK_OP_INC = REGISTER_BANK_OP
REGISTER_BANK_OP_DEC = 2 * REGISTER_BANK_OP

REGISTER_DATA127 = allocate_bit(1)
...
# jal instruction: r127<=PC, PC<=imm16
def generate_jal(opcode):
    start = opcode * OPCODE_SIZE
    microcode[start] = 0
    microcode[start+1] = NEXT
    microcode[start+2] = NEXT | REGISTERS_WR_SOURCE_SET127 | REGISTER_BANK_OP_INC
    microcode[start+3] = PC_SOURCE_IMMEDIATE | RAM_ADDR_SOURCE_IMMEDIATE | REGISTERS_WR | REGISTERS_WR_DATA_SOURCE_PC | STAGE_RESET

# jalr instruction: r127<=PC, PC<=ry
def generate_jalr(opcode):
    start = opcode * OPCODE_SIZE
    microcode[start] = 0
    microcode[start+1] = NEXT | REGISTERS_WR_SOURCE_SET127
    microcode[start+2] = REGISTER_BANK_OP_INC
    microcode[start+3] = PC_SOURCE_REGISTER | RAM_ADDR_SOURCE_REGISTER | REGISTERS_WR | REGISTERS_WR_DATA_SOURCE_PC | STAGE_RESET
...
# ret instruction: PC<=r127, register_bank_no--
def generate_ret(opcode):
    start = opcode * OPCODE_SIZE
    microcode[start] = 0
    microcode[start+1] = REGISTER_DATA127
    microcode[start+2] = PC_SOURCE_REGISTER | RAM_ADDR_SOURCE_REGISTER | REGISTER_BANK_OP_DEC | STAGE_RESET
