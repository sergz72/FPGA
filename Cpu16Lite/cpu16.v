...
(
    ...
    function [15:0] registers_wr_source_f(input [2:0] source);
        case (source)
            ...
            3: registers_wr_source_f = registers_data2 + {{8{1'b0}}, current_instruction[23: 16]};
    ...
    function [7:0] registers_wr_address_f(input [1:0] source);
        case (source)
            ...
            1: registers_wr_address_f = current_instruction[23:16];
            2: registers_wr_address_f = sp;
            3: registers_wr_address_f = rp_op == 2 ? rp - 1: rp;
        endcase
    endfunction
    ...
    function [7:0] registers_rd_source1_f(input [1:0] source);
        case (source)
            ...
            default: registers_rd_source1_f = rp;
        endcase
    endfunction
    ...
    assign registers_rd_address2 = current_instruction[31:25] == 7'h7F ? (current_instruction[24] ? rp - 1 : rp) : current_instruction[31:24];
    ...
    assign registers_wr_dest = current_microinstruction[18:17];
    assign registers_wr_source = current_microinstruction[21:19];
    assign rp_op = current_microinstruction[23:22];
    assign registers_rd_source1 = current_microinstruction[25:24];
    ...
    assign io_address = registers_data1 + {{8{1'b0}}, current_instruction[BITS * 2 - 1: 24]};
                            ...
                            if (hlt == 0) begin
                                ...
                                if ((address_load == 0) || (condition_pass == 0))
                                    ...
                                    if (pop) begin
                                        ...
                                        address <= address_source ? current_instruction[BITS * 2 - 1:BITS] : registers_data1 + {8'h00, current_instruction[31:24]};
                                    end
                                end
                                case (rp_op)
